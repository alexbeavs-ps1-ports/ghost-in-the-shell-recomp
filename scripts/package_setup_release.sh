...
#
# Writes: dist/ghost-in-the-shell-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix ghost-in-the-shell \
  --exe-name Ghost_in_the_Shell_Recompiled \
  --display-name "Ghost in the Shell Recompiled" \
